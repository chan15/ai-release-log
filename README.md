...
這是一個自動化的 GitHub Release 監控與通知工具，專門追蹤熱門 AI CLI 工具（如 Gemini, Copilot, Codex, Claude Code）的更新資訊。它會自動抓取最新版本、利用 Gemini AI 翻譯，並發送美化後的訊息到 Discord 頻道。

## ✨ 特色功能

- **可擴充架構**：採用 **Factory Pattern**，只需繼承 `BaseScraper` 並實作特定規則，即可輕鬆新增追蹤專案。
- **多專案追蹤**：預設支援 Gemini CLI, Copilot CLI, OpenAI Codex 與 Claude Code。
...
#    Supported vendors: gemini, copilot, codex, claude
```
...
- 版本檔案讀取/儲存邏輯
- 不同發布風格的解析邏輯（Gemini、Copilot、Codex、Claude Code）
...
- `scrapers/`：抓取邏輯目錄（Factory Pattern 實作）。
    ...
    - `gemini.py`, `copilot.py`, `codex.py`, `claude.py`：具體的專案設定與抓取規則。
- `tests/`：單元測試目錄。
